...
bool BPstate;
bool BPstate_old = false; //Machine d'état du BP
int iState =0;
float fVoltage;
int iADCval;
int iRandRGB;
...
int main (void){
    ...
    while(true!=false){
        //bool b24hDone =false;
        bool b24hDone = timer1.wait24h();
        BPstate = SW2.GetState();
        if(BPstate==true && BPstate_old==false){ //Premier Appui
            BPstate_old = true;
        }
        if ((BPstate == false && BPstate_old == true)||b24hDone) { //Relachement ou 24h elapsed
            BPstate_old = false;
            
            //Read Inputs
            iADCval = AN1.ReadADC();
            fVoltage = ((iADCval/4096.0)*2 +10.5); //[10.5V ; 12.5V]
            
            //Update State
            iState = Processing::processVoltage(fVoltage); 

            //Update Outputs
            Processing::UpdateLedState(iState,LED,rgb); 
            ...
            //Turn off MCU
            Processing::UpdateLedState(0,LED,rgb); 
        }
